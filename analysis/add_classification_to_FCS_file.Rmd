...
# Directory names
df_all_files <- cbind("path" = v_all_files, 
                      "directory_name" = dirname(v_all_files),
                      "file_name" = basename(v_all_files))
# Split path names # TO ADAPT with classification of the user's dataset
tb_classified_files <- separate(as.tibble(df_all_files), 
                                col = directory_name, 
                                into = c("nb_bacteria", "subgroup"), 
                                sep = "/",
                                fill = "right",
                                remove = FALSE)
```
...
# Convert group to number # TO ADAPT with the data's classification
tb_classified_files <- tb_classified_files %>% 
  mutate(manual_classification = 
           case_when(nb_bacteria == "0_bacteria" ~ 0,
                     nb_bacteria == "1_bacterium" ~ 1,
                     nb_bacteria == "2_bacteria" ~ 2,
                     nb_bacteria == "3_bacteria" ~ 3,
                     nb_bacteria == "4_bacteria" ~ 4,
                     nb_bacteria == "5_and_above" ~ 5))
```
...
# Create a vector
# Fill the vector with value to know if there is an image that was generated or not
# if WaveformPresent parameter is different than 0, an image was generated
## 111: image was generated
## 112: no image was generated
v_col <- mutate(as_tibble(exprs(original)), 
                     image_TF = ifelse(WaveformPresent != 0, 111, 112)) %>% 
  pull(image_TF)
...
table_manualclassification <- unique(tb_classified_files[,c("nb_bacteria", 
                              "manual_classification")])
colnames(table_manualclassification)[1] <- "groups"
# Table summarizing the code to know if an image was generated or not
## Create based on the WaveformPresent parameters
## 111: image was generated
## 112: no image was generated
table_image <- data.frame("groups" = c("yes_image", "no_image"),
           "manual_classification" = c(111, 112))
# Merge the two tables
table_classcode <- rbind(table_manualclassification,
                         table_image)
```
